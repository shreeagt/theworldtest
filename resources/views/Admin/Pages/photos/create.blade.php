...
<form action="{{ route('photos.store') }}" method="POST" enctype="multipart/form-data">
    ...
    @if (isset($photo_details))
  
    <input hidden value="{{$photo_details->id}}" name="photo_id"/>
    @endif
    <div class="form-group">
      <label for="exampleInputEmail1">Photo Title</label>
      <input type="text" class="form-control" name="photo_title" required value="{{ isset($photo_details)?$photo_details->photos_title:'' }}" aria-describedby="emailHelp" placeholder="Enter Photo Title">

    </div>
    <div class="form-group">
      <label for="exampleInputPassword1">Photo URL</label>
      <input type="text" class="form-control" name="photos_url" required value="{{isset($photo_details)?$photo_details->photos_url:''}}" {{isset($photo_details)?'readonly':''}}  placeholder="Enter Photo URL">
    </div>
    <div class="form-group">
      <label for="exampleInputPassword1">Select Photo Category</label>
    <select name="photo_category_id" class="form-control" required>
      <option value=""></option>
      @foreach ($photo_categories as $item)
      <option {{isset($photo_details)?$photo_details->photos_category_id==$item->id?'selected':'':''}} value="{{$item->id}}">{{$item->category_title}}</option>
    ...
    <div class="form-group">
        <label for="exampleInputPassword1">Photo Tag</label>
        <input type="text" class="form-control" id="photo_tag" name="photo_tag" value="{{isset($photo_details)?$photo_details->photos_tag:''}}"  placeholder="Enter Photo Tag">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Photo Description</label>
        <input type="text" class="form-control" name="photo_description" value="{{isset($photo_details)?$photo_details->photos_description:''}}" placeholder="Enter Photo Description">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Photo Keyword</label>
        <input type="text" class="form-control" id="photo_keyword" name="photo_keyword" value="{{isset($photo_details)?$photo_details->photos_keyword:''}}" placeholder="Enter Photo Keywords">
      </div>

      <div class="form-group">
        <label for="editor">Photo Body</label>
        <textarea class="editor" name="editor" id="editor">{!! isset($photo_details)?$photo_details->photos_body:'' !!}</textarea>
      </div>

      <div class="form-group">
        <label for="editor">Photo Thumbnail</label>
        <input type="file" name="photo_thumbnail" {{isset($photo_details)?'':'required'}} class="form-control"/>
        <img src="{{isset($photo_details)?$photo_details->photos_thumbnail:''}}" style="height: 200px;" alt=""/>
      </div>
      <div class="form-group">
        <label for="editor">Photo Banner Type</label>
        <select name="photo_banner_type" class="form-control">
          <option value="Image" {{isset($photo_details)?$photo_details->photos_banner_type=='Image'?'selected':'':''}}>Image</option>
          <option value="Video" {{isset($photo_details)?$photo_details->photos_banner_type=='Video'?'selected':'':''}}>Video</option>
        </select>
      </div>
      <div class="form-group">
        <label for="editor">Photo Banner</label>
        <input type="file" name="photo_banner" class="form-control" {{isset($photo_details)?'':'required'}}/>
        <img src="{{isset($photo_details)?$photo_details->photos_banner:''}}" style="height: 200px;" alt=""/>
      </div>
      <div class="form-group">
        <label for="editor">Photo Status</label>
        <select name="photo_status" class="form-control">
          <option value="Draft" {{isset($photo_details)?$photo_details->photos_status=='Draft'?'selected':'':''}}>Draft</option>
          <option value="Publish" {{isset($photo_details)?$photo_details->photos_status=='Publish'?'selected':'':''}}>Publish</option>
