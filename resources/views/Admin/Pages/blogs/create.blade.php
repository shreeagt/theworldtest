...
<form action="{{route('blog.store')}}" method="POST" enctype="multipart/form-data">
  ...
  <input hidden value="{{$blog_details->id}}" name="blog_id"/>
  @endif
    <div class="form-group">
      <label for="exampleInputEmail1">Blog Title</label>
      <input type="text" class="form-control" name="blog_title" required  placeholder="Enter Blog Title" value="{{isset($blog_details)?$blog_details->blog_title:''}}">

    </div>
    <div class="form-group">
      <label for="exampleInputPassword1">Blog URL</label>
      <input type="text" class="form-control" name="blog_url" required placeholder="Enter Blog URL" value="{{isset($blog_details)?$blog_details->blog_url:''}}"  {{isset($blog_details)?'readonly':''}}>
    </div>
    <div class="form-group">
      <label for="exampleInputPassword1">Select Blog Category</label>
    <select name="blog_category_id" class="form-control" required>
      <option value=""></option>
      @foreach ($blog_categories as $item)
      <option {{isset($blog_details)?$blog_details->blog_category_id==$item->id?'selected':'':''}} value="{{$item->id}}">{{$item->category_title}}</option>
    ...
    <div class="form-group">
        <label for="exampleInputPassword1">Blog Tag</label>
        <input type="text" class="form-control" id="blog_tag" name="blog_tag"   placeholder="Enter Blog Tag" value="{{isset($blog_details)?$blog_details->blog_tag:''}}">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Blog Description</label>
        <input type="text" class="form-control" name="blog_description" required  placeholder="Enter Blog Description" value="{{isset($blog_details)?$blog_details->blog_description:''}}">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Blog Keyword</label>
        <input type="text" class="form-control" id="blog_keyword" name="blog_keyword"  placeholder="Enter Blog Keywords" value="{{isset($blog_details)?$blog_details->blog_keyword:''}}">
      </div>

      <div class="form-group">
        <label for="editor">Blog Body</label>
        <textarea class="editor" name="editor" id="editor">{!! isset($blog_details)?$blog_details->blog_body:'' !!}</textarea>
      </div>

      <div class="form-group">
        <label for="blog_thumbnail">Blog Thumbnail</label>
        <input type="file" name="blog_thumbnail" class="form-control" {{isset($blog_details)?'':'required'}}/>
        <img src="{{isset($blog_details)?$blog_details->blog_thumbnail:''}}" style="height: 200px;" alt=""/>
      </div>
      <div class="form-group">
        <label for="banner_type">Blog Banner Type</label>
        <select name="blog_banner_type" class="form-control" required>
          <option {{isset($blog_details)?$blog_details->blog_banner_type=='Image'?'selected':'':''}} value="Image">Image</option>
          <option {{isset($blog_details)?$blog_details->blog_banner_type=='Video'?'selected':'':''}} value="Video">Video</option>
        </select>
      </div>
      <div class="form-group">
        <label for="blog_banner">Blog Banner</label>
        <input type="file" name="blog_banner" class="form-control" {{isset($blog_details)?'':'required'}}/>
        <img src="{{isset($blog_details)?$blog_details->blog_banner:''}}" style="height: 200px;" alt=""/>
      </div>
      <div class="form-group">
        <label for="editor">Blog Status</label>
        <select name="blog_status" class="form-control" required>
          <option {{isset($blog_details)?$blog_details->blog_status=='Draft'?'selected':'':''}} value="Draft">Draft</option>
          <option {{isset($blog_details)?$blog_details->blog_status=='Publish'?'selected':'':''}} value="Publish">Publish</option>
