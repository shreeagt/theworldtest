...
<form action="{{ route('videos.store') }}" method="POST" enctype="multipart/form-data">
    ...
    @if (isset($videos_details))
  
    <input hidden value="{{$videos_details->id}}" name="video_id"/>
    @endif
    <div class="form-group">
      <label for="exampleInputEmail1">Video Title</label>
      <input type="text" class="form-control" name="video_title" required value="{{ isset($videos_details)?$videos_details->video_title:'' }}" aria-describedby="emailHelp" placeholder="Enter Video Title">

    </div>
    <div class="form-group">
      <label for="exampleInputPassword1">Video URL</label>
      <input type="text" class="form-control" name="video_url" required value="{{isset($videos_details)?$videos_details->video_url:''}}" {{isset($videos_details)?'readonly':''}}  placeholder="Enter Video URL">
    </div>
    <div class="form-group">
      <label for="exampleInputPassword1">Select Video Category</label>
    <select name="video_category_id" class="form-control" required>
      <option value=""></option>
      @foreach ($video_categories as $item)
      <option {{isset($videos_details)?$videos_details->video_category_id==$item->id?'selected':'':''}} value="{{$item->id}}">{{$item->category_title}}</option>
    ...
    <div class="form-group">
        <label for="exampleInputPassword1">Video Tag</label>
        <input type="text" class="form-control" id="video_tag" name="video_tag" value="{{isset($videos_details)?$videos_details->video_tag:''}}"  placeholder="Enter Video Tag">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Video Description</label>
        <input type="text" class="form-control" name="video_description" value="{{isset($videos_details)?$videos_details->video_description:''}}" placeholder="Enter Video Description">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Video Keyword</label>
        <input type="text" class="form-control" id="video_keyword" name="video_keyword" value="{{isset($videos_details)?$videos_details->video_keyword:''}}" placeholder="Enter Video Keywords">
      </div>

      <div class="form-group">
        <label for="editor">Video Body</label>
        <textarea class="editor" name="editor" id="editor">{!! isset($videos_details)?$videos_details->video_body:'' !!}</textarea>
      </div>

      <div class="form-group">
        <label for="editor">Video Thumbnail</label>
        <input type="file" name="video_thumbnail" {{isset($videos_details)?'':'required'}} class="form-control"/>
        <img src="{{isset($videos_details)?$videos_details->video_thumbnail:''}}" style="height: 200px;" alt=""/>
      </div>
      <div class="form-group">
        <label for="editor">Video Banner Type</label>
        <select name="video_banner_type" class="form-control">
          <option value="Image" {{isset($videos_details)?$videos_details->video_banner_type=='Image'?'selected':'':''}}>Image</option>
          <option value="Video" {{isset($videos_details)?$videos_details->video_banner_type=='Video'?'selected':'':''}}>Video</option>
        </select>
      </div>
      <div class="form-group">
        <label for="editor">Video Banner</label>
        <input type="file" name="video_banner" class="form-control" {{isset($videos_details)?'':'required'}}/>
        <img src="{{isset($videos_details)?$videos_details->video_banner:''}}" style="height: 200px;" alt=""/>
      </div>
      <div class="form-group">
        <label for="editor">Video Status</label>
        <select name="video_status" class="form-control">
          <option value="Draft" {{isset($videos_details)?$videos_details->video_status=='Draft'?'selected':'':''}}>Draft</option>
          <option value="Publish" {{isset($videos_details)?$videos_details->video_status=='Publish'?'selected':'':''}}>Publish</option>
